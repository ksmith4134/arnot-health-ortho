    ...
    return (
        <div onClick={() => openModal(url)} className='w-full h-full relative block overflow-hidden rounded-md hover:cursor-pointer group'>
            <Image 
                ...
                className='object-cover group-hover:scale-105 transition ease-in-out duration-500 aspect-video brightness-110' 
            />
            <div className='absolute top-0 bottom-0 left-0 right-0 flex justify-center items-center'>
                <Image 
                    src={playButton} alt='play button'
                    className='w-20 aspect-square lg:opacity-90 group-hover:opacity-100 transition ease-in-out duration-500 bg-white/30 rounded-full' 
                />
            </div>
        </div>
    )
